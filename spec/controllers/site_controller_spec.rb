...
describe SitesController do
  ...
  describe "the create method" do
    ...
    def setup_aws_mocks_shallow_save
      @file_mock = double(AWS::S3::S3Object)

      @file_mock.should_receive(:acl=).exactly(1).times.with(:public_read)
      @file_mock.should_receive(:public_url).exactly(1).times {@only_url_in_the_doc}
    ...
    context "with the correct user" do
      context "with the shallow save flag set to false" do
        before do
          setup_aws_mocks_deep_save
        end
        ...
        it "should save @import tags correctly" do
          @file_mock.should_receive(:write).once
      ...
      context "with the shallow save flag set to true"  do
        before do
          setup_aws_mocks_shallow_save
        end

        it "should only make one request to aws, with the html as the argument" do
          @file_mock.should_receive(:write).exactly(1).times do |args|
            args.index("html").should_not == nil
          end
          make_shallow_save_create_request
        end
