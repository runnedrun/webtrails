HighlightManager = function(trackedDoc, trackedIframe, currentSite) {
    var $doc = $(trackedDoc);
    ...
    function highlightedTextDetect(){
        ...
        $doc.unbind("mouseup");
        if (!rangy.getIframeSelection(trackedIframe).isCollapsed){
    ...
    function addComment(highlightedTextRange){
        currentCommentCreator = new CommentCreator(0, highlightedTextRange, trackedDoc, currentSite);
    }

    function getHighlightedTextRange(){
        return rangy.getIframeSelection(trackedIframe).getRangeAt(0);
    }
