...
;(function ($) {
    ...
    var Completer = (function () {
        ...
        $baseList = applyDefaultCSS($(html.list)).css(css.list);
        _id = 0;
        ...
        function Completer($el, option) {
            ...
            this.strategies = [];
            this.option = option;
            this.prefix = option.classNamePrefix || ""
            if (focus) {
                this.initialize();
                this.$el.focus();
            } else {
            ...
            $baseList.extend(option.listStyles || {});
        }
        ...
        $.extend(Completer.prototype, {
             ...
             */
            getCaretRelativePosition:function(){var b,c,e,f,g,h,i,j,k;return"true"!=this.el.contentEditable?(b=["border-width","font-family","font-size","font-style","font-variant","font-weight","height","letter-spacing","word-spacing","line-height","text-decoration","text-align","width","padding-top","padding-right","padding-bottom","padding-left","margin-top","margin-right","margin-bottom","margin-left","border-style","box-sizing"],i=this.$el[0].scrollHeight>this.$el[0].offsetHeight,c=$.extend({position:"absolute",overflow:i?"scroll":"auto","white-space":"pre-wrap",top:0,left:-9999,direction:h},d(this.$el,b)),e=$("<div></div>").css(c).text(this.getTextFromHeadToCaret()),f=$("<span></span>").text(".").appendTo(e),this.$el.before(e),g=f.position(),g.top+=f.height()-this.$el.scrollTop(),e.remove()):(j=window.getSelection().getRangeAt(0).cloneRange(),k=document.createElement("span"),j.insertNode(k),j.selectNodeContents(k),j.deleteContents(),g=$(k).position(),g.top+=$(k).height()-this.$el.scrollTop()),h=this.$el.attr("dir")||this.$el.css("direction"),"rtl"===h&&(g.left-=this.listView.$el.width()),g},
    ...
    var ListView = (function () {
        ...
        function ListView($el, completer) {
            ...
            this.$el.on('mousedown.textComplete', 'li.' + completer.prefix + 'textcomplete-item',
        ...
        $.extend(ListView.prototype, {
            ...
            render: function (data) {
                ...
                if(this.strategy.header) {
                    ...
                    html += '<li class="' + this.completer.prefix + "textcomplete-header" + '">' + str + '</li>';
                }
                ...
                for (i = 0, l = data.length; i < l; i++) {
                    ...
                    this.data.push(val);
                    html += '<li class="' + this.completer.prefix  + "textcomplete-item" + '" data-index="' + index + '"><a>';
                ...
                if(this.strategy.footer) {
                    ...
                    html += '<li class="' + this.completer.prefix  + "textcomplete-item" + '">' + str + '</li>';
                }

                var $html = $(html);
                applyDefaultCSS($html.contents()).css(InlineBootstrap.listItem);

                var itemStyle = this.completer.option.itemStyle;
                if (itemStyle) {
                    $html.css(itemStyle);
                }
            ...
            activateIndexedItem: function () {
                this.$el.find('.' + this.completer.prefix + 'active').removeClass(this.completer.prefix + 'active')
                    .css(InlineBootstrap.listItem)
                    .find("a")
                    .css(InlineBootstrap.listItem);
                this.getActiveItem().addClass(this.completer.prefix + 'active');
                var activeStyle = this.completer.option.activeStyle;
                if (activeStyle) {
                    this.getActiveItem().find("a").css(activeStyle);
                    this.getActiveItem().css(activeStyle);
                }
            },

            getActiveItem: function () {
                return $(this.$el.children('.' + this.completer.prefix + 'textcomplete-item').get(this.index));
            },
            ...
            setPosition: function (position) {
                ...
                if (this.completer.option.dropdownPadding) {
                    position.top = position.top + (this.completer.option.dropdownPadding.top || 0)
                    position.left = position.left + (this.completer.option.dropdownPadding.left || 0)
                }
            ...
            onClick: function (e) {
                ...
                if (!$e.hasClass(this.completer.prefix + 'textcomplete-item')) {
                    $e = $e.parents('li.' + this.completer.prefix + 'textcomplete-item');
                }
